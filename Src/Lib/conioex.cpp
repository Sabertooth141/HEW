...
static const char palette256Esc[] = "\x1b]4;000;rgb:00/00/00\x1b\\";
//２５６パレット用RGB設定エスケープシーケンス：ESC[4;<パレット番号：10進数3ケタ;rgb:<RR>/<GG>/<BB>ESC\> ←RGB値は各16進2ケタ
...
bool InitD2D(int _window_width, int _window_height)
{
	...
	hr = g_pD2DFactory->CreateHwndRenderTarget(D2D1::RenderTargetProperties(),
	                                           D2D1::HwndRenderTargetProperties(
		                                           g_hConWnd, D2D1_SIZE_U{
			                                           (UINT32) _window_width, (UINT32) _window_height
		                                           }), &g_pRenderTarget);
	_ASSERT(hr == S_OK);
#else
	/*
	...
	*/
	...
	// Debug: warn if client rect doesn't match requested size
	_RPTN(_CRT_WARN, "InitD2D: requested=%dx%d, client=%ldx%ld\n",
	      _window_width, _window_height, cr.right, cr.bottom);
...
static void push_screen_buffer(void)
{
	HRESULT hr = ~S_OK;
	D2D1_SIZE_U siz;
	siz.width = g_ScreenBufferSize.X;
	siz.height = g_ScreenBufferSize.Y;
	...
	hr = pD2D1_Bmp->CopyFromMemory(&ru, reinterpret_cast<const void *>(g_FrameBuffer32bitD2D),
	                               g_ScreenBufferSize.X * 4);
	_ASSERT(hr == S_OK);
	//■■■■ 描画したビットマップをリストに追加 ■■■
...
void WriteTextA(int _xp, int _yp, const char *_text, double _scale, D2D1::ColorF _fgc, D2D1::ColorF _bgc,
                bool _new_line)
{
...
void WriteTextW(int _xp, int _yp, const wchar_t *_text, double _scale, D2D1::ColorF _fgc, D2D1::ColorF _bgc,
                bool _new_line)
{
	if (_scale < 1.0)
	{
	...
	//互換ビットマップのレンダーターゲットを作成（テキスト領域サイズのみ確保）
	...
	// 四角形の描画
	p_bitmap_render_target->Clear(D2D1::ColorF(0, 0, 0, 0));
...
void InitConioEx(int _width, int _height, int _font_w, int _font_h, const wchar_t *_font_face_name,
                 const COLORREF *_pal16, bool _init_wbuf)
{
	...
	g_ScreenBufferInfoEx.wAttributes = (FOREGROUND_INTENSITY | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
	//画面バッファーの文字属性:文字=パレット#15／背景=パレット#0
...
void PrintStringA(const char *_srcbuf, int _size)
{
	...
	WriteTextA(g_CursorPosD2D.X, g_CursorPosD2D.Y, _srcbuf, 1.0f, D2D1::ColorF(1, 1, 1, 1), D2D1::ColorF(0, 0, 0, 0),
	           false);
} //PrintStringA
...
void PrintStringW(const wchar_t *_srcbuf, int _size)
{
	...
	g_PrintStringCompatibleMode = true;
	WriteTextW(g_CursorPosD2D.X, g_CursorPosD2D.Y, _srcbuf, 1.0f, D2D1::ColorF(1, 1, 1, 1), D2D1::ColorF(0, 0, 0, 0),
	           false);
} //PrintStringW
...
void PrintString(const char *_src, int _size)
{
	...
	g_PrintStringCompatibleMode = true;
	WriteTextW(g_CursorPosD2D.X, g_CursorPosD2D.Y, wc_src, 1.0f, D2D1::ColorF(1, 1, 1, 1), D2D1::ColorF(0, 0, 0, 0),
	           false);
} //PrintString
#endif	//UNICODE
...
void DeleteLine(void)
{
	...
	for (calc_coord.Y = g_ScreenBufferInfoEx.dwCursorPosition.Y; calc_coord.Y < g_ScreenBufferInfoEx.srWindow.Bottom;
	     calc_coord.Y++)
	{
		calc_coord.Y++;
...
void DrawStringFA(int _x, int _y, bool _zenkaku, const char *_format, ...)
{
	...
	g_CursorPosD2D.Y = _y;
#else
	SetConsoleCursorPosition(g_DisplayHandle[g_SwapFlg],
	                         COORD{(SHORT) _x, (SHORT) (g_ScreenBufferInfoEx.srWindow.Top + _y)});
#endif // USED2D
	va_list ap;
	va_start(ap, _format);
	VPrintStringFA(_zenkaku, _format, ap);
...
{
	RGBQUAD rgb = {
		(BYTE) ((ref & 0x00FF0000) >> 16)/*Blue*/, (BYTE) ((ref & 0x0000FF00) >> 8)/*Green*/,
		(BYTE) (ref & 0x000000FF)/*Red*/, 0x00/*Reserved*/
	};
	return rgb;
} //CREFtoRGBQ
...
inline void pixel_copy24(const unsigned char *in_buf, int xx, int yy, int, bool _tr)
{
	if ((xx >= 0) && (yy >= 0) && (yy < g_ScreenBufferSize.Y) && (xx < g_ScreenBufferSize.X))
	{
		...
		{
			//透明指定が無しor透明ピクセルでは無い
			RGBQUAD *dest = &g_FrameBuffer32bitD2D[yy * g_ScreenBufferSize.X + xx];
			dest->rgbBlue = in_buf[0];
			dest->rgbGreen = in_buf[1];
			dest->rgbRed = in_buf[2];
			dest->rgbReserved = 0;
		}
	}
	//in_buf += 3;
...
Bmp *LoadBmp(const char *_file_name, bool _palset_or_swap24RB)
{
	...
	if (bih.biBitCount >= 1 && bih.biBitCount <= 8)
	{
		...
		{
			pb->pal[n] = (pal_rgb[n].rgbRed & 0x0000FF) | ((pal_rgb[n].rgbGreen << 8) & 0x00FF00) | (
				             (pal_rgb[n].rgbBlue << 16) & 0xFF0000);
		}
#else
		//RGB値を表すRGBQUAD値の配列をパレット数の数だけ確保する
		//１色はDWORDでバイト並び順は[B][G][R][A]として格納されている
		...
		for (int n = 0; n < pb->numpal; n++)
		{
			pb->pal[n] = (pal_rgb4[n].rgbRed & 0x0000FF) | ((pal_rgb4[n].rgbGreen << 8) & 0x00FF00) | (
				             (pal_rgb4[n].rgbBlue << 16) & 0xFF0000);
		}
#endif // USED2D
	}
	...
	switch (pb->colbit)
	{
		case 1: bpp_num = BPP_1;
			break;
		case 4: bpp_num = BPP_4;
			break;
		case 8: bpp_num = BPP_8;
			break;
		case 16: bpp_num = BPP_16;
			break;
		case 24: bpp_num = BPP_24;
			break;
		case 32: bpp_num = BPP_32;
			break;
		default: bpp_num = BPP_8;
			break;
	}
...
*/
bool LoadDivBmp(const char *_path, int _x0, int _y0, size_t _xpix, size_t _ypix, size_t _xcount, size_t _ycount,
                Bmp **_pp_bmp)
{
...
const DWORD Gray65[65] = {
	//======== START ========
	0x00000000,
	0x03030303, 0x07070707, 0x0B0b0b0b, 0x0F0f0f0f, 0x13131313, 0x17171717, 0x1B1b1b1b, 0x1F1f1f1f, 0x23232323,
	0x27272727, 0x2B2b2b2b, 0x2F2f2f2f, 0x33333333, 0x37373737, 0x3B3b3b3b, 0x3F3f3f3f, //[16]
	0x43434343, 0x47474747, 0x4B4b4b4b, 0x4F4f4f4f, 0x53535353, 0x57575757, 0x5B5b5b5b, 0x5F5f5f5f, 0x63636363,
	0x67676767, 0x6B6b6b6b, 0x6F6f6f6f, 0x73737373, 0x77777777, 0x7B7b7b7b, 0x7F7f7f7f, //[32]
	0x83838383, 0x87878787, 0x8B8b8b8b, 0x8F8f8f8f, 0x93939393, 0x97979797, 0x9B9b9b9b, 0x9F9f9f9f, 0xA3a3a3a3,
	0xA7a7a7a7, 0xABababab, 0xAFafafaf, 0xB3b3b3b3, 0xB7b7b7b7, 0xBBbbbbbb, 0xBFbfbfbf, //[48]
	0xC3c3c3c3, 0xC7c7c7c7, 0xCBcbcbcb, 0xCFcfcfcf, 0xD3d3d3d3, 0xD7d7d7d7, 0xDBdbdbdb, 0xDFdfdfdf, 0xE3e3e3e3,
	0xE7e7e7e7, 0xEBebebeb, 0xEFefefef, 0xF3f3f3f3, 0xF7f7f7f7, 0xFBfbfbfb, 0xFFffffff, //[64]
	//======== END ========
};
...
Bmp *CreateBmpChar(const TCHAR *_font_name, int _font_size, int _bold, int _ggo, const TCHAR *_text)
{
	...
	{
		default: //２値
		case GGO_BITMAP: aa_level = 2;
			break; //２値
		case GGO_GRAY2_BITMAP: aa_level = 5;
			break; //５階調
		case GGO_GRAY4_BITMAP: aa_level = 17;
			break; //１７階調
		case GGO_GRAY8_BITMAP: aa_level = 65;
			break; //６５階調
	}
			...
			for (unsigned int count = 0; count < read; count++, input_record++)
			{
				switch (input_record->EventType)
				{
					case KEY_EVENT:
					{
						...
						{
							g_ConioKeyMap[key_event->wVirtualKeyCode >> 5] |= (
								0x01 << (key_event->wVirtualKeyCode & 31));
						} else
						{
							g_ConioKeyMap[key_event->wVirtualKeyCode >> 5] &= ~(
								0x01 << (key_event->wVirtualKeyCode & 31));
...
void MciSetVolume(int *sound_id, int percent)
{
	...
	switch (sound_info->device_type)
	{
		case MCI_DEVTYPE_DIGITAL_VIDEO:
			...
			mciSendCommand(sound_info->device_id, MCI_SETAUDIO, MCI_DGV_SETAUDIO_ITEM | MCI_DGV_SETAUDIO_VALUE,
			               (DWORD_PTR) &audio_param);
			break;
		case MCI_DEVTYPE_SEQUENCER:
...
void PrintImage(const char *_buf)
{
	...
	WriteConsoleA(g_DisplayHandle[g_SwapFlg], g_ScreenBufferFull,
	              (g_ScreenBufferSize.X * PIXEL24ESCSIZE) * g_ScreenBufferSize.Y + (g_ScreenBufferSize.Y - 1),
	              &write_num, NULL);
} //PrintImage
#endif // !USED2D
...
const COLORREF ANSI_PAL256_COLOR[NUM_ANSI_PAL] = {
	...
	0x005FFF, 0x008700, 0x00875F, 0x008787, 0x0087AF,
	0x0087D7, 0x0087FF, 0x00AF00, 0x00AF5F, 0x00AF87, 0x00AFAF, 0x00AFD7, 0x00AFFF, 0x00D700, 0x00D75F, 0x00D787,
	0x00D7AF, 0x00D7D7, 0x00D7FF, 0x00FF00, 0x00FF5F,
	0x00FF87, 0x00FFAF, 0x00FFD7, 0x00FFFF, 0x5F0000, 0x5F005F, 0x5F0087, 0x5F00AF, 0x5F00D7, 0x5F00FF, 0x5F5F00,
	0x5F5F5F, 0x5F5F87, 0x5F5FAF, 0x5F5FD7, 0x5F5FFF,
	0x5F8700, 0x5F875F, 0x5F8787, 0x5F87AF, 0x5F87D7, 0x5F87FF, 0x5FAF00, 0x5FAF5F, 0x5FAF87, 0x5FAFAF, 0x5FAFD7,
	0x5FAFFF, 0x5FD700, 0x5FD75F, 0x5FD787, 0x5FD7AF,
	0x5FD7D7, 0x5FD7FF, 0x5FFF00, 0x5FFF5F, 0x5FFF87, 0x5FFFAF, 0x5FFFD7, 0x5FFFFF, 0x870000, 0x87005F, 0x870087,
	0x8700AF, 0x8700D7, 0x8700FF, 0x875F00, 0x875F5F,
	0x875F87, 0x875FAF, 0x875FD7, 0x875FFF, 0x878700, 0x87875F, 0x878787, 0x8787AF, 0x8787D7, 0x8787FF, 0x87AF00,
	0x87AF5F, 0x87AF87, 0x87AFAF, 0x87AFD7, 0x87AFFF,
	0x87D700, 0x87D75F, 0x87D787, 0x87D7AF, 0x87D7D7, 0x87D7FF, 0x87FF00, 0x87FF5F, 0x87FF87, 0x87FFAF, 0x87FFD7,
	0x87FFFF, 0xAF0000, 0xAF005F, 0xAF0087, 0xAF00AF,
	0xAF00D7, 0xAF00FF, 0xAF5F00, 0xAF5F5F, 0xAF5F87, 0xAF5FAF, 0xAF5FD7, 0xAF5FFF, 0xAF8700, 0xAF875F, 0xAF8787,
	0xAF87AF, 0xAF87D7, 0xAF87FF, 0xAFAF00, 0xAFAF5F,
	0xAFAF87, 0xAFAFAF, 0xAFAFD7, 0xAFAFFF, 0xAFD700, 0xAFD75F, 0xAFD787, 0xAFD7AF, 0xAFD7D7, 0xAFD7FF, 0xAFFF00,
	...
	0xD75FFF, 0xD78700, 0xD7875F, 0xD78787, 0xD787AF,
	0xD787D7, 0xD787FF, 0xD7AF00, 0xD7AF5F, 0xD7AF87, 0xD7AFAF, 0xD7AFD7, 0xD7AFFF, 0xD7D700, 0xD7D75F, 0xD7D787,
	0xD7D7AF, 0xD7D7D7, 0xD7D7FF, 0xD7FF00, 0xD7FF5F,
	0xD7FF87, 0xD7FFAF, 0xD7FFD7, 0xD7FFFF, 0xFF0000, 0xFF005F, 0xFF0087, 0xFF00AF, 0xFF00D7, 0xFF00FF, 0xFF5F00,
	0xFF5F5F, 0xFF5F87, 0xFF5FAF, 0xFF5FD7, 0xFF5FFF,
	0xFF8700, 0xFF875F, 0xFF8787, 0xFF87AF, 0xFF87D7, 0xFF87FF, 0xFFAF00, 0xFFAF5F, 0xFFAF87, 0xFFAFAF, 0xFFAFD7,
	0xFFAFFF, 0xFFD700, 0xFFD75F, 0xFFD787, 0xFFD7AF,
	0xFFD7D7, 0xFFD7FF, 0xFFFF00, 0xFFFF5F, 0xFFFF87, 0xFFFFAF, 0xFFFFD7, 0xFFFFFF, 0x080808, 0x121212, 0x1C1C1C,
	0x262626, 0x303030, 0x3A3A3A, 0x444444, 0x4E4E4E,
	0x585858, 0x626262, 0x6C6C6C, 0x767676, 0x808080, 0x8A8A8A, 0x949494, 0x9E9E9E, 0xA8A8A8, 0xB2B2B2, 0xBCBCBC,
...
const RGBQUAD ANSI_PAL256_RGB[NUM_ANSI_PAL] = {
	{0x00, 0x00, 0x00, 0}, {0x80, 0x00, 0x00, 0}, {0x00, 0x80, 0x00, 0}, {0x80, 0x80, 0x00, 0}, {0x00, 0x00, 0x80, 0},
	{0x80, 0x00, 0x80, 0}, {0x00, 0x80, 0x80, 0}, {0xC0, 0xC0, 0xC0, 0}, {0x80, 0x80, 0x80, 0}, {0xFF, 0x00, 0x00, 0},
	{0x00, 0xFF, 0x00, 0}, {0xFF, 0xFF, 0x00, 0}, {0x00, 0x00, 0xFF, 0}, {0xFF, 0x00, 0xFF, 0}, {0x00, 0xFF, 0xFF, 0},
	{0xFF, 0xFF, 0xFF, 0},
	{0x00, 0x00, 0x00, 0}, {0x00, 0x00, 0x5F, 0}, {0x00, 0x00, 0x87, 0}, {0x00, 0x00, 0xAF, 0}, {0x00, 0x00, 0xD7, 0},
	{0x00, 0x00, 0xFF, 0}, {0x00, 0x5F, 0x00, 0}, {0x00, 0x5F, 0x5F, 0}, {0x00, 0x5F, 0x87, 0}, {0x00, 0x5F, 0xAF, 0},
	{0x00, 0x5F, 0xD7, 0}, {0x00, 0x5F, 0xFF, 0}, {0x00, 0x87, 0x00, 0}, {0x00, 0x87, 0x5F, 0}, {0x00, 0x87, 0x87, 0},
	{0x00, 0x87, 0xAF, 0},
	{0x00, 0x87, 0xD7, 0}, {0x00, 0x87, 0xFF, 0}, {0x00, 0xAF, 0x00, 0}, {0x00, 0xAF, 0x5F, 0}, {0x00, 0xAF, 0x87, 0},
	{0x00, 0xAF, 0xAF, 0}, {0x00, 0xAF, 0xD7, 0}, {0x00, 0xAF, 0xFF, 0}, {0x00, 0xD7, 0x00, 0}, {0x00, 0xD7, 0x5F, 0},
	{0x00, 0xD7, 0x87, 0}, {0x00, 0xD7, 0xAF, 0}, {0x00, 0xD7, 0xD7, 0}, {0x00, 0xD7, 0xFF, 0}, {0x00, 0xFF, 0x00, 0},
	{0x00, 0xFF, 0x5F, 0},
	{0x00, 0xFF, 0x87, 0}, {0x00, 0xFF, 0xAF, 0}, {0x00, 0xFF, 0xD7, 0}, {0x00, 0xFF, 0xFF, 0}, {0x5F, 0x00, 0x00, 0},
	{0x5F, 0x00, 0x5F, 0}, {0x5F, 0x00, 0x87, 0}, {0x5F, 0x00, 0xAF, 0}, {0x5F, 0x00, 0xD7, 0}, {0x5F, 0x00, 0xFF, 0},
	{0x5F, 0x5F, 0x00, 0}, {0x5F, 0x5F, 0x5F, 0}, {0x5F, 0x5F, 0x87, 0}, {0x5F, 0x5F, 0xAF, 0}, {0x5F, 0x5F, 0xD7, 0},
	{0x5F, 0x5F, 0xFF, 0},
	{0x5F, 0x87, 0x00, 0}, {0x5F, 0x87, 0x5F, 0}, {0x5F, 0x87, 0x87, 0}, {0x5F, 0x87, 0xAF, 0}, {0x5F, 0x87, 0xD7, 0},
	{0x5F, 0x87, 0xFF, 0}, {0x5F, 0xAF, 0x00, 0}, {0x5F, 0xAF, 0x5F, 0}, {0x5F, 0xAF, 0x87, 0}, {0x5F, 0xAF, 0xAF, 0},
	{0x5F, 0xAF, 0xD7, 0}, {0x5F, 0xAF, 0xFF, 0}, {0x5F, 0xD7, 0x00, 0}, {0x5F, 0xD7, 0x5F, 0}, {0x5F, 0xD7, 0x87, 0},
	{0x5F, 0xD7, 0xAF, 0},
	{0x5F, 0xD7, 0xD7, 0}, {0x5F, 0xD7, 0xFF, 0}, {0x5F, 0xFF, 0x00, 0}, {0x5F, 0xFF, 0x5F, 0}, {0x5F, 0xFF, 0x87, 0},
	{0x5F, 0xFF, 0xAF, 0}, {0x5F, 0xFF, 0xD7, 0}, {0x5F, 0xFF, 0xFF, 0}, {0x87, 0x00, 0x00, 0}, {0x87, 0x00, 0x5F, 0},
	{0x87, 0x00, 0x87, 0}, {0x87, 0x00, 0xAF, 0}, {0x87, 0x00, 0xD7, 0}, {0x87, 0x00, 0xFF, 0}, {0x87, 0x5F, 0x00, 0},
	{0x87, 0x5F, 0x5F, 0},
	{0x87, 0x5F, 0x87, 0}, {0x87, 0x5F, 0xAF, 0}, {0x87, 0x5F, 0xD7, 0}, {0x87, 0x5F, 0xFF, 0}, {0x87, 0x87, 0x00, 0},
	{0x87, 0x87, 0x5F, 0}, {0x87, 0x87, 0x87, 0}, {0x87, 0x87, 0xAF, 0}, {0x87, 0x87, 0xD7, 0}, {0x87, 0x87, 0xFF, 0},
	{0x87, 0xAF, 0x00, 0}, {0x87, 0xAF, 0x5F, 0}, {0x87, 0xAF, 0x87, 0}, {0x87, 0xAF, 0xAF, 0}, {0x87, 0xAF, 0xD7, 0},
	{0x87, 0xAF, 0xFF, 0},
	{0x87, 0xD7, 0x00, 0}, {0x87, 0xD7, 0x5F, 0}, {0x87, 0xD7, 0x87, 0}, {0x87, 0xD7, 0xAF, 0}, {0x87, 0xD7, 0xD7, 0},
	{0x87, 0xD7, 0xFF, 0}, {0x87, 0xFF, 0x00, 0}, {0x87, 0xFF, 0x5F, 0}, {0x87, 0xFF, 0x87, 0}, {0x87, 0xFF, 0xAF, 0},
	{0x87, 0xFF, 0xD7, 0}, {0x87, 0xFF, 0xFF, 0}, {0xAF, 0x00, 0x00, 0}, {0xAF, 0x00, 0x5F, 0}, {0xAF, 0x00, 0x87, 0},
	{0xAF, 0x00, 0xAF, 0},
	{0xAF, 0x00, 0xD7, 0}, {0xAF, 0x00, 0xFF, 0}, {0xAF, 0x5F, 0x00, 0}, {0xAF, 0x5F, 0x5F, 0}, {0xAF, 0x5F, 0x87, 0},
	{0xAF, 0x5F, 0xAF, 0}, {0xAF, 0x5F, 0xD7, 0}, {0xAF, 0x5F, 0xFF, 0}, {0xAF, 0x87, 0x00, 0}, {0xAF, 0x87, 0x5F, 0},
	{0xAF, 0x87, 0x87, 0}, {0xAF, 0x87, 0xAF, 0}, {0xAF, 0x87, 0xD7, 0}, {0xAF, 0x87, 0xFF, 0}, {0xAF, 0xAF, 0x00, 0},
	{0xAF, 0xAF, 0x5F, 0},
	{0xAF, 0xAF, 0x87, 0}, {0xAF, 0xAF, 0xAF, 0}, {0xAF, 0xAF, 0xD7, 0}, {0xAF, 0xAF, 0xFF, 0}, {0xAF, 0xD7, 0x00, 0},
	{0xAF, 0xD7, 0x5F, 0}, {0xAF, 0xD7, 0x87, 0}, {0xAF, 0xD7, 0xAF, 0}, {0xAF, 0xD7, 0xD7, 0}, {0xAF, 0xD7, 0xFF, 0},
	{0xAF, 0xFF, 0x00, 0}, {0xAF, 0xFF, 0x5F, 0}, {0xAF, 0xFF, 0x87, 0}, {0xAF, 0xFF, 0xAF, 0}, {0xAF, 0xFF, 0xD7, 0},
	{0xAF, 0xFF, 0xFF, 0},
	{0xD7, 0x00, 0x00, 0}, {0xD7, 0x00, 0x5F, 0}, {0xD7, 0x00, 0x87, 0}, {0xD7, 0x00, 0xAF, 0}, {0xD7, 0x00, 0xD7, 0},
	{0xD7, 0x00, 0xFF, 0}, {0xD7, 0x5F, 0x00, 0}, {0xD7, 0x5F, 0x5F, 0}, {0xD7, 0x5F, 0x87, 0}, {0xD7, 0x5F, 0xAF, 0},
	{0xD7, 0x5F, 0xD7, 0}, {0xD7, 0x5F, 0xFF, 0}, {0xD7, 0x87, 0x00, 0}, {0xD7, 0x87, 0x5F, 0}, {0xD7, 0x87, 0x87, 0},
	{0xD7, 0x87, 0xAF, 0},
	{0xD7, 0x87, 0xD7, 0}, {0xD7, 0x87, 0xFF, 0}, {0xD7, 0xAF, 0x00, 0}, {0xD7, 0xAF, 0x5F, 0}, {0xD7, 0xAF, 0x87, 0},
	{0xD7, 0xAF, 0xAF, 0}, {0xD7, 0xAF, 0xD7, 0}, {0xD7, 0xAF, 0xFF, 0}, {0xD7, 0xD7, 0x00, 0}, {0xD7, 0xD7, 0x5F, 0},
	{0xD7, 0xD7, 0x87, 0}, {0xD7, 0xD7, 0xAF, 0}, {0xD7, 0xD7, 0xD7, 0}, {0xD7, 0xD7, 0xFF, 0}, {0xD7, 0xFF, 0x00, 0},
	{0xD7, 0xFF, 0x5F, 0},
	{0xD7, 0xFF, 0x87, 0}, {0xD7, 0xFF, 0xAF, 0}, {0xD7, 0xFF, 0xD7, 0}, {0xD7, 0xFF, 0xFF, 0}, {0xFF, 0x00, 0x00, 0},
	{0xFF, 0x00, 0x5F, 0}, {0xFF, 0x00, 0x87, 0}, {0xFF, 0x00, 0xAF, 0}, {0xFF, 0x00, 0xD7, 0}, {0xFF, 0x00, 0xFF, 0},
	{0xFF, 0x5F, 0x00, 0}, {0xFF, 0x5F, 0x5F, 0}, {0xFF, 0x5F, 0x87, 0}, {0xFF, 0x5F, 0xAF, 0}, {0xFF, 0x5F, 0xD7, 0},
	{0xFF, 0x5F, 0xFF, 0},
	{0xFF, 0x87, 0x00, 0}, {0xFF, 0x87, 0x5F, 0}, {0xFF, 0x87, 0x87, 0}, {0xFF, 0x87, 0xAF, 0}, {0xFF, 0x87, 0xD7, 0},
	{0xFF, 0x87, 0xFF, 0}, {0xFF, 0xAF, 0x00, 0}, {0xFF, 0xAF, 0x5F, 0}, {0xFF, 0xAF, 0x87, 0}, {0xFF, 0xAF, 0xAF, 0},
	{0xFF, 0xAF, 0xD7, 0}, {0xFF, 0xAF, 0xFF, 0}, {0xFF, 0xD7, 0x00, 0}, {0xFF, 0xD7, 0x5F, 0}, {0xFF, 0xD7, 0x87, 0},
	{0xFF, 0xD7, 0xAF, 0},
	{0xFF, 0xD7, 0xD7, 0}, {0xFF, 0xD7, 0xFF, 0}, {0xFF, 0xFF, 0x00, 0}, {0xFF, 0xFF, 0x5F, 0}, {0xFF, 0xFF, 0x87, 0},
	{0xFF, 0xFF, 0xAF, 0}, {0xFF, 0xFF, 0xD7, 0}, {0xFF, 0xFF, 0xFF, 0}, {0x08, 0x08, 0x08, 0}, {0x12, 0x12, 0x12, 0},
	{0x1C, 0x1C, 0x1C, 0}, {0x26, 0x26, 0x26, 0}, {0x30, 0x30, 0x30, 0}, {0x3A, 0x3A, 0x3A, 0}, {0x44, 0x44, 0x44, 0},
	{0x4E, 0x4E, 0x4E, 0},
	{0x58, 0x58, 0x58, 0}, {0x62, 0x62, 0x62, 0}, {0x6C, 0x6C, 0x6C, 0}, {0x76, 0x76, 0x76, 0}, {0x80, 0x80, 0x80, 0},
	{0x8A, 0x8A, 0x8A, 0}, {0x94, 0x94, 0x94, 0}, {0x9E, 0x9E, 0x9E, 0}, {0xA8, 0xA8, 0xA8, 0}, {0xB2, 0xB2, 0xB2, 0},
	{0xBC, 0xBC, 0xBC, 0}, {0xC6, 0xC6, 0xC6, 0}, {0xD0, 0xD0, 0xD0, 0}, {0xDA, 0xDA, 0xDA, 0}, {0xE4, 0xE4, 0xE4, 0},
	{0xEE, 0xEE, 0xEE, 0},
};
...
void DrawPixelBuffer256(int _xp, int _yp, int _w, int _h, BYTE *_buf)
{
	...
	DWORD wrn;
	WriteConsoleA(g_DisplayHandle[g_SwapFlg], g_ScreenBuffer256, screen_area_pixel_size * sizeof(pixel256Esc), &wrn,
	              NULL);
} //SetPixel256
#endif // !USED2D
...
void FrameSync(void)
{
	...
	if (f_wait > 0)
	{
		// Hybrid wait: sleep for bulk of wait, spin for final precision
		double f_sleep_threshold = f_1sec * 0.002; // ~2ms in rdtsc counts
		if (f_wait > f_sleep_threshold)
		{
			double ms_to_sleep = ((f_wait - f_sleep_threshold) / f_1sec) * 1000.0;
			if (ms_to_sleep >= 1.0)
			{
				Sleep((DWORD) ms_to_sleep);
			}
		}
		// Spin for the remaining ~2ms for frame-accurate timing
		__int64 t2 = 0;
		do
		{
			t2 = __rdtsc(); //現在時間取得
	...
	if ((f_total >= f_1sec) && (dbg_frame_count > 0))
	{
		...
		sprintf_s(dbg_str, sizeof(dbg_str),
		          ...
		          (double) fps, (int) dbg_frame_count, (double) f_1sec, (double) f_tpf, (double) f_wait,
		          (double) f_frame_interval, (double) f_total);
